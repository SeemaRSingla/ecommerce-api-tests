...
app.post('/graphql', (req, res) => {
  ...
  if (hasOp('SearchProducts')) {
    ...
    if (variables && variables.searchTerm) {
      searchResults = searchResults.filter((p) =>
        p.name.toLowerCase().includes(String(variables.searchTerm).toLowerCase())
      );
    }
    const response = {
      data: {
        searchProducts: Array.isArray(searchResults) ? searchResults : [],
  ...
  if (hasOp('ProductsByCategory')) {
    ...
    if (variables && variables.category)
      filtered = filtered.filter((p) => p.category === variables.category);
    if (variables && variables.minPrice)
      filtered = filtered.filter((p) => p.price >= variables.minPrice);
    if (variables && variables.maxPrice)
      filtered = filtered.filter((p) => p.price <= variables.maxPrice);
    const response = {
      data: { productsByCategory: Array.isArray(filtered) ? filtered : [], count: filtered.length },
    };
    res.setHeader('Content-Type', 'application/json');
    res.json(response);
    return;
  ...
  if (hasOp('GetProducts')) {
    ...
    const response = {
      data: { products: Array.isArray(result) ? result : [], productCount: result.length },
    };
    res.setHeader('Content-Type', 'application/json');
    res.json(response);
    return;
